...
export default function PenjualanPage() {
  ...
  const fetchTransactions = async () => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase.from('transactions').select('id, created_at, total_amount, amount_paid, status, contact:contacts(name)').eq('type', 'SO_OUTBOUND').order('created_at', { ascending: false });
  ...
  return (
    <div className="p-4 md:p-8 max-w-full overflow-hidden">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 md:mb-8">
        ...
        <Link href="/penjualan/buat" className="bg-purple-700 hover:bg-purple-800 text-white border-2 border-purple-900 px-5 py-3 rounded-lg text-base font-bold flex items-center gap-2">
             ...
             transactions.map(tx => (
              <tr key={tx.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 flex justify-center">
                  <Link href={`/penjualan/${tx.id}`} className="text-white bg-gray-800 hover:bg-black px-4 py-2 rounded-lg font-bold border-2 border-black">Detail</Link>
                </td>
              </tr>
            ))}
          </tbody>
