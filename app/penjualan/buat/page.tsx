...
import { ArrowLeft, Plus, Trash2, Save, Search, ShoppingCart } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-100 p-4 md:p-8">
      <form onSubmit={handleSubmit} className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-black text-black uppercase tracking-tight">Buat Penjualan (SO)</h1>
        </div>
          ...
          <div className="">
            <table className="w-full text-left border-collapse min-w-[700px]">
              ...
              <tbody className="divide-y divide-gray-100">
                {lines.map((line, idx) => {
                  const suggestions = line.searchTerm.length >= 1
                    ? allProducts.filter(p => p.name.toLowerCase().includes(line.searchTerm.toLowerCase())).slice(0, 8)
                    : [];
                  return (
                    <tr key={line.uid} className="hover:bg-gray-50/50">
                      ...
                      <td className="py-3 px-4 align-top relative">
                        <input
                          ...
                          onFocus={() => updateLine(line.uid, { showSuggestions: true })}
                        ...
                        />
                        {line.showSuggestions && line.searchTerm.length >= 1 && (
                          <div className="absolute left-0 right-0 top-full mt-1 z-[9999] bg-white border-2 border-purple-600 rounded-xl shadow-2xl max-h-60 overflow-y-auto">
                              ...
                              suggestions.map(p => (
                                <button key={p.id} type="button"
                                  ...
                                  <div className="flex-1 flex justify-between items-center min-w-0">
                                    ...
                                    <span className="shrink-0 text-[10px] font-black text-purple-600 bg-purple-50 px-2 py-0.5 rounded uppercase ml-2 border border-purple-100">
                      ...
                      <td className="py-3 px-4 align-top">
                        <input type="number" step="any" min="0" value={line.quantity}
                          ...
                          className="w-full px-3 py-2.5 border-2 border-gray-200 rounded-lg text-black font-bold focus:border-purple-600 outline-none text-sm text-center" placeholder="0" />
                      </td>
                      <td className="py-3 px-4 align-top">
                        <input type="text" value={formatNum(line.harga_perkilo)}
                          ...
                          className="w-full px-3 py-2.5 border-2 border-gray-200 rounded-lg text-black font-bold focus:border-purple-600 outline-none text-sm text-right" placeholder="0" />
                      </td>
                      <td className="py-3 px-4 align-top">
