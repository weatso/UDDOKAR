...
import { Search, Archive, Package, AlertTriangle, CheckCircle, XCircle, ArrowLeft } from 'lucide-react';
import Link from 'next/link';

export default function StokBarangMentahPage() {
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-black flex items-center gap-3">
            <Package className="w-8 h-8 text-blue-600" /> Stok Barang Mentah
          </h1>
          <p className="text-gray-500 font-semibold mt-1">Pemantauan stok kertas, tinta, lem, dan bahan baku lainnya.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6 items-center justify-between">
        <div className="relative w-full md:w-96">
          ...
          <input
            type="text"
            placeholder="Cari bahan baku (kertas, tinta, dll)..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-10 pr-4 py-2.5 border-2 border-gray-200 rounded-2xl focus:border-blue-500 focus:outline-none font-semibold text-black"
          />
        </div>
        <div className="flex gap-2">
          <div className="bg-blue-50 px-4 py-2 rounded-xl border border-blue-200 text-xs font-bold text-blue-700">
      ...
      <div className="bg-white rounded-3xl border-2 border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          <div className="p-20 text-center text-gray-400 font-bold italic">Menghubungkan ke gudang...</div>
        ) : filteredProducts.length === 0 ? (
          <div className="p-20 text-center">
            <Archive className="w-12 h-12 text-gray-200 mx-auto mb-4" />
            <p className="text-gray-400 font-bold">Tidak ada barang mentah ditemukan.</p>
          </div>
        ) : (
          <>
            {/* Desktop Table */}
            <div className="hidden md:block">
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-50 border-b-2 border-gray-100">
                  <tr>
                    <th className="px-6 py-4 font-black text-gray-400 text-[10px] uppercase tracking-widest">Barang & Kategori</th>
                  ...
                  {filteredProducts.map((p) => (
                    <tr key={p.id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <td className="px-6 py-5 text-center">
                        <span className={`text-2xl font-black ${p.stock_quantity <= 0 ? 'text-red-600' : p.stock_quantity <= 10 ? 'text-orange-600' : 'text-blue-700'}`}>
                          {p.stock_quantity}
                        </span>
                ...
                <div key={p.id} className="p-5 rounded-2xl border-2 border-gray-100 bg-gray-50">
                  ...
                  <div className="flex justify-between items-center border-t border-gray-200 pt-3 mt-3">
                    ...
                    <div className="text-right">
                      ...
                      <p className={`text-xl font-black ${p.stock_quantity <= 0 ? 'text-red-600' : p.stock_quantity <= 10 ? 'text-orange-600' : 'text-blue-700'}`}>
                        {p.stock_quantity} <span className="text-xs text-gray-400 uppercase font-bold">Unit</span>
                      </p>
                    </div>
                  </div>
                </div>
