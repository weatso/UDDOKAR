...
import { Plus, Trash2, Save, Search, ShoppingCart, Box, History } from 'lucide-react';
...
export default function PembelianUtamaPage() {
    ...
    <div className="min-h-screen bg-gray-100 p-4 md:p-8">
      <form onSubmit={handleSubmit} className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <Link href="/pembelian/riwayat" className="w-full md:w-auto bg-white hover:bg-gray-50 text-gray-800 border-2 border-gray-300 px-6 py-3 rounded-xl text-sm font-black flex items-center justify-center gap-2 shadow-sm transition-all uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-gray-100">
                {lines.map((line, idx) => (
                  <tr key={line.uid} className="hover:bg-gray-50/50">
                    ...
                    <td className="py-3 px-4 align-top">
                      <div className="relative">
                        <input type="text" value={line.product_id ? line.name : line.searchTerm}
                          ...
                          onFocus={(e) => onInputFocus(line.uid, e)} placeholder="Ketik nama bahan..."
                        ...
                        {typeof document !== 'undefined' && line.showSuggestions && activeLineUid === line.uid && inputRect && createPortal(
                          <div ref={dropdownRef} className="fixed z-[9999] bg-white border-2 border-blue-600 rounded-xl shadow-2xl max-h-60 overflow-y-auto overflow-x-hidden"
                              ...
                              if (suggestions.length > 0) return suggestions.map(p => (
                                <button key={p.id} type="button" onMouseDown={e => { e.preventDefault(); selectProduct(line.uid, p); }}
                                  ...
                                  <div className="flex-1 flex justify-between items-center min-w-0">
                                    ...
                                    <span className="shrink-0 text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase ml-2 border border-blue-100">Stok: {p.stock_quantity}</span>
                                  </div>
                                </button>
                              ));
                    ...
                    <td className="py-3 px-4 align-top"><input type="number" step="any" min="0" value={line.quantity} onChange={e => updateLine(line.uid, { quantity: e.target.value })} className="w-full px-3 py-2.5 border-2 border-gray-200 rounded-lg text-black font-bold focus:border-blue-600 outline-none text-sm text-center" placeholder="0" /></td>
                    <td className="py-3 px-4 align-top"><input type="number" step="any" min="0" value={line.tonase} onChange={e => updateLine(line.uid, { tonase: e.target.value })} className="w-full px-3 py-2.5 border-2 border-gray-200 rounded-lg text-black font-bold focus:border-blue-600 outline-none text-sm text-center" placeholder="0" /></td>
              ...
              <div key={line.uid} className="p-4 space-y-3 bg-white">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-black text-blue-700">{formatRp(lineTotal(line))}</span>
                    <button type="button" onClick={() => setLines(prev => prev.length <= 1 ? [emptyLine()] : prev.filter(l => l.uid !== line.uid))} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg border border-red-100"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Deskripsi Bahan</label>
                  <div className="relative">
                    <input type="text" value={line.product_id ? line.name : line.searchTerm}
                      onChange={e => { updateLine(line.uid, { searchTerm: e.target.value, showSuggestions: true, product_id: '', name: '' }); setInputRect(e.target.getBoundingClientRect()); }}
                      onFocus={(e) => onInputFocus(line.uid, e)} placeholder="Ketik nama bahan..."
                      className="w-full px-3 py-3 border-2 border-gray-200 rounded-xl text-black font-semibold focus:border-blue-600 outline-none text-sm" />
                    {typeof document !== 'undefined' && line.showSuggestions && activeLineUid === line.uid && inputRect && createPortal(
                      <div ref={dropdownRef} className="fixed z-[9999] bg-white border-2 border-blue-600 rounded-xl shadow-2xl max-h-60 overflow-y-auto overflow-x-hidden"
                        ...
                        {(() => {
                          ...
                          if (suggestions.length > 0) return suggestions.map(p => (
                            <button key={p.id} type="button" onMouseDown={e => { e.preventDefault(); selectProduct(line.uid, p); }}
                              className="w-full px-4 py-3 text-left hover:bg-blue-50 flex items-center gap-3 border-b border-gray-100 last:border-0 transition-colors">
                              <div className="flex-1 flex justify-between items-center min-w-0">
                                <span className="font-bold text-black text-sm truncate">{p.name}</span>
                                <span className="shrink-0 text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase ml-2">Stok: {p.stock_quantity}</span>
                              </div>
                            </button>
                          ));
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input type="number" step="any" min="0" value={line.quantity} onChange={e => updateLine(line.uid, { quantity: e.target.value })} className="w-full px-3 py-3 border-2 border-gray-200 rounded-xl text-black font-bold focus:border-blue-600 outline-none text-sm text-center" placeholder="0" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Tonase</label>
                    <input type="number" step="any" min="0" value={line.tonase} onChange={e => updateLine(line.uid, { tonase: e.target.value })} className="w-full px-3 py-3 border-2 border-gray-200 rounded-xl text-black font-bold focus:border-blue-600 outline-none text-sm text-center" placeholder="0" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Harga /kg</label>
                    <input type="text" value={formatNum(line.harga_perkilo)} onChange={e => updateLine(line.uid, { harga_perkilo: e.target.value.replace(/[^0-9]/g, '') })} className="w-full px-3 py-3 border-2 border-gray-200 rounded-xl text-black font-bold focus:border-blue-600 outline-none text-sm text-right" placeholder="0" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Harga Satuan</label>
                    <input type="text" value={formatNum(line.harga_satuan)} onChange={e => updateLine(line.uid, { harga_satuan: e.target.value.replace(/[^0-9]/g, '') })} className="w-full px-3 py-3 border-2 border-gray-200 rounded-xl text-black font-bold focus:border-blue-600 outline-none text-sm text-right" placeholder="0" />
                  </div>
                </div>
              </div>
            ))}
