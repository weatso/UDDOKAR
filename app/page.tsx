  ...
  useEffect(() => {
    async function fetchChartData() {
      ...
      try {
        ...
        const { data: txHistory, error } = await supabase
          .from('transactions')
        ...
        // For 1y, we might want to group by month to avoid too many points
        let finalData = Object.values(dailyData);
        if (timeframe === '1y') {
          const monthlyData: { [key: string]: { date: string, pemasukan: number, pengeluaran: number } } = {};
          finalData.forEach(d => {
            const monthStr = d.date.substring(0, 7); // YYYY-MM
            if (!monthlyData[monthStr]) monthlyData[monthStr] = { date: monthStr + "-01", pemasukan: 0, pengeluaran: 0 };
            monthlyData[monthStr].pemasukan += d.pemasukan;
            monthlyData[monthStr].pengeluaran += d.pengeluaran;
          });
